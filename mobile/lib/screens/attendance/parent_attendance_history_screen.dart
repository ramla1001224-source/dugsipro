...
class _ParentAttendanceHistoryScreenState extends State<ParentAttendanceHistoryScreen> {
      ...
      if (profile != null && profile['Parent'] != null) {
        ...
        if (childrenData != null) {
          ...
          if (_children.isNotEmpty) {
            ...
            _selectedStudentName = _children[0]['user']?['name'] ?? _children[0]['name'] ?? 'Student';
            await _loadAttendance();
          } else {
  ...
  Future<void> _loadAttendance() async {
    ...
    try {
      ...
      final res = await _api.get('${ApiConfig.childAttendance}/$_selectedStudentId', params: params);
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: _children.map((child) {
                  String name = child['user']?['name'] ?? child['name'] ?? 'Student';
                ...
                onChanged: (v) {
                  if (v != null) {
                    setState(() {
                      ...
                      _selectedStudentName = _children.firstWhere((c) => c['id'] == v)['user']?['name'] ?? _children.firstWhere((c) => c['id'] == v)['name'] ?? 'Student';
                    });
                    _loadAttendance();
                  }
                },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('${_selectedStudentName ?? "Student"} - Attendance'),
      ),
      body: Column(
        children: [
