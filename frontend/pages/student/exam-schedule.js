...
export default function StudentExamSchedule() {
    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            ...
            try {
                const profileRes = await axios.get(`${apiUrl}/api/auth/profile`, { headers: headers() })
                const profileData = profileRes.data;
                const studentProfile = profileData?.Student || profileData;
                const currentEnrollment = profileData?.currentEnrollment || studentProfile?.Enrollments?.[0];
                
                const classId = currentEnrollment?.classId || studentProfile?.classId;
                const sectionId = currentEnrollment?.sectionId || studentProfile?.sectionId;

                if (!classId) { setLoading(false); return }

                const res = await axios.get(
                    `${apiUrl}/api/exams?classId=${classId}${sectionId ? `&sectionId=${sectionId}` : ''}&onlyCurrent=true`,
                    { headers: headers() }
                )
                const data = Array.isArray(res.data) ? res.data : (res.data.data || [])
                const scheduled = data.filter(ex => ex.date).sort((a, b) => new Date(a.date) - new Date(b.date))
                setTotalCount(scheduled.length)
        ...
        <Layout title="Jadwalka Imtixaanka">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                    ...
                    <p className="text-gray-400 font-medium text-sm uppercase tracking-widest mt-1">Qorshaha Imtixaanadaada — Grouped by Term</p>
                </div>
                {totalCount > 0 && (
                                            ...
                                            return (
                                                <tr key={ex.id} className={`border-b border-slate-50 transition-colors ${isToday ? 'bg-orange-50/60 hover:bg-orange-50' : idx % 2 === 0 ? 'bg-white hover:bg-indigo-50/30' : 'bg-slate-50/30 hover:bg-indigo-50/30'} ${isPast ? 'opacity-60' : ''}`}>
                                                    ...
                                                    <td className="px-6 py-5">
                                                        ...
                                                        {ex.description && <p className="text-[9px] font-bold text-amber-500 mt-1 italic">{ex.description}</p>}
                                                    </td>
                                                    <td className="px-6 py-5">
                                                    ...
                                                    <td className="px-6 py-5"><span className="text-sm font-black text-slate-700">{examDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></td>
