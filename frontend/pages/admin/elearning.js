...
export default function AdminQuizzes() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Flatten hierarchical classes for the form
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingQuizId) {
                await axios.put(`${apiUrl}/api/elearning/quizzes/${editingQuizId}`, formData, { headers: getHeaders() })
                alert('Quiz updated successfully!')
            } else {
                await axios.post(`${apiUrl}/api/elearning/quizzes`, formData, { headers: getHeaders() })
    ...
    return (
        <Layout title={t('elearning')}>
            {viewingResultQuiz && (
                <div className="absolute inset-0 bg-white z-50 p-8 min-h-screen animate-in fade-in slide-in-from-bottom-8">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex justify-between items-center mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-slate-800">{t('exam_results')}: {viewingResultQuiz.title}</h2>
                                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">{viewingResultQuiz.results?.length} Submissions</p>
                            </div>
                            <button onClick={() => setViewingResultQuiz(null)} className="bg-slate-50 hover:bg-slate-200 text-slate-600 px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all">← {t('cancel')}</button>
                        </div>
                        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-xl overflow-hidden">
                            {viewingResultQuiz.results?.length === 0 ? (
                                <div className="p-20 text-center text-slate-400 font-bold uppercase tracking-widest">{t('no_data')}</div>
                            ) : (
                                <table className="w-full text-left">
                                    <thead className="bg-slate-50 border-b border-slate-100">
                                        <tr>
                                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('name')}</th>
                                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('score')}</th>
                                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('status')}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
            ...
            <div className={`transition-all ${viewingResultQuiz ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
                <div className="flex justify-between items-center mb-8">
                <div>
                    <h2 className="text-2xl font-black text-slate-800">{t('elearning')} {t('management')}</h2>
            ...
            {showCreate ? (
                <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-xl mb-12 animate-in fade-in slide-in-from-bottom-4">
                    <div className="flex justify-between items-center mb-10">
                        ...
                        <button onClick={() => { setShowCreate(false); setEditingQuizId(null); setFormData({ title: '', classId: '', sectionId: '', subjectId: '', duration: 30, questions: [{ question: '', options: ['', '', '', ''], correctAnswer: 0, points: 1 }] }) }} className="bg-slate-50 p-3 px-6 rounded-xl hover:bg-red-50 hover:text-red-500 transition-all font-bold text-slate-500">{t('cancel')}</button>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">{t('subject')}</label>
                ...
                <div className="flex gap-4 mb-8">
                    ...
                    <button onClick={() => setActiveTab('history')} className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-slate-800 text-white shadow-xl' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>{t('history')} ({quizzes.filter(q => q.isActive === false).length})</button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {loading ? (
                        <div className="col-span-full text-center py-20 text-slate-400 font-bold uppercase tracking-widest">{t('loading')}</div>
                        ...
                        <div key={q.id} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-xl shadow-slate-100 hover:scale-[1.02] transition-all group">
                            ...
                            <div className="space-y-2 mb-8">
                                ...
                                <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest"><span>👥</span> {q.clss?.class_name} ({q.section?.name})</div>
                                ...
                                <div className="flex items-center gap-2 text-[10px] font-black text-blue-500 uppercase tracking-widest pt-2 border-t border-slate-50"><span>👨‍🏫</span> By: {q.teacher?.user?.name || 'Admin'}</div>
                            </div>
                            
                            <div className="flex gap-2 mb-2">
                                <button onClick={() => handleViewResults(q)} className="flex-1 py-3 bg-slate-50 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-100 transition-all">{t('results_label')}</button>
                                <button onClick={() => handleEditQuiz(q)} className="flex-1 py-3 bg-blue-50 text-blue-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-100 transition-all">{t('edit')}</button>
